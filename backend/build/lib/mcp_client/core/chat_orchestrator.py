...
class ChatOrchestrator:
    ...
    async def process_query(self, query: str, session) -> AsyncGenerator[Dict[str, Any], None]:
            ...
            while not is_response_ready:
                ...
                for content in response.choices:
                    if content.finish_reason == "stop":
                        text_content = content.message.content or ""
                        if text_content:
                        ...
                        if tools_called:
                            yield {"type": "reasoning", "text": f"I will use {len(tools_called)} tool(s) to answer the query."}
                        ...
                        for tool in tools_called:
                            ...
                            yield {
                                "type": "tools",
                                "header": {"type": tool_name, "state": "output-available"},
                                "input": {"query": parsed_args},
                                "output": str(result),
                            }
    ...
    async def chat_loop(self, session, manager, websocket: Optional[WebSocket]):
        ...
        while True:
            try:
                ...
                async for part in self.process_query(query, session):
                    response_message = {
                        "id": message_id,
                        "role": "assistant",
                        "parts": [part],
